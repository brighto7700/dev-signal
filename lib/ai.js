...
export async function summarizeWithAI(input, url = null) {
  ...
  if (Array.isArray(input)) {
    const briefContext = input.map(s => `- ${s.title}`).join('\n');
    return await callOpenRouter(
      "You are ShellSignal AI. Summarize these stories into a single, sharp 3-bullet technical brief.",
      briefContext
    );
  }
  ...
  const summary = await callOpenRouter(
    "You are ShellSignal AI. Give me 3 technical, witty bullet points for this story.",
    title
  );
